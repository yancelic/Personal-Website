import { useRef } from "react";
import { motion } from "framer-motion";
import { useInView } from "../hooks/useInView";

const socials = [
  {
    label: "GitHub",
    href: "https://github.com/EchoKatana",
    icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/></svg>,
  },
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/in/myankilic/",
    icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>,
  },
  {
    label: "Instagram",
    href: "https://instagram.com/yankeelic",
    icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>,
  },
];

const quotes = {
  en: "I'm always open to ambitious projects, honest conversations, and ideas that don't quite fit in a box.",
  tr: "Her zaman iddialı projelere, dürüst sohbetlere ve bir kutuya tam sığmayan fikirlere açığım.",
};

export default function ContactSection({ lang }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true });

  return (
    <section id="contact" style={{ position: "relative" }}>
      {/* Hardware mounting screws */}
      <div className="hardware-screw" style={{ position: "absolute", top: "20px", left: "20px", opacity: 0.5 }} />
      <div className="hardware-screw" style={{ position: "absolute", top: "20px", right: "20px", opacity: 0.5 }} />
      <div className="hardware-screw" style={{ position: "absolute", bottom: "20px", left: "20px", opacity: 0.5 }} />
      <div className="hardware-screw" style={{ position: "absolute", bottom: "20px", right: "20px", opacity: 0.5 }} />

      {/* Giant ghost text behind */}
      <div className="contact-giant-text">
        {lang === "en" ? "Let's\nTalk" : "Konuş\nalım"}
      </div>

      <div className="contact-inner-wrap" ref={ref}>
        {/* Overline */}
        <motion.div
          className="contact-overline"
          initial={{ opacity: 0, y: 16 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6 }}
        >
          <div className="section-label">{lang === "en" ? "Get In Touch" : "İletişime Geçin"}</div>
        </motion.div>

        {/* Main headline */}
        <motion.h2
          className="contact-headline"
          initial={{ opacity: 0, y: 30 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.8, delay: 0.1, ease: [0.25, 1, 0.5, 1] }}
        >
          {lang === "en"
            ? <>Let's build<br />something <em>remarkable</em></>
            : <>Birlikte <em>olağanüstü</em><br />bir şey inşa edelim</>}
        </motion.h2>

        {/* Sub quote */}
        <motion.p
          className="contact-sub"
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : {}}
          transition={{ duration: 0.8, delay: 0.3 }}
        >
          {quotes[lang]}
        </motion.p>

        {/* CTA row */}
        <motion.div
          className="contact-cta-row"
          initial={{ opacity: 0, y: 16 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6, delay: 0.45 }}
        >
          <a href="mailto:myanki.work@gmail.com" className="btn-primary" style={{ fontSize: "0.82rem", padding: "1rem 3rem" }}>
            {lang === "en" ? "Send a message" : "Mesaj gönder"}
          </a>
          <span className="contact-email crt-display" style={{ display: "inline-block", color: "var(--red)", textShadow: "0 0 6px var(--red-glow)" }}>
            myanki.work@gmail.com
          </span>
        </motion.div>

        {/* Divider */}
        <motion.div
          className="contact-divider"
          initial={{ scaleX: 0 }}
          animate={isInView ? { scaleX: 1 } : {}}
          transition={{ duration: 1, delay: 0.5, ease: "easeInOut" }}
          style={{ transformOrigin: "left" }}
        />

        {/* Social links */}
        <div className="contact-socials">
          {socials.map((s, i) => (
            <motion.a
              key={i}
              href={s.href}
              className="contact-social-link"
              target="_blank"
              rel="noreferrer"
              initial={{ opacity: 0, y: 12 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: 0.6 + i * 0.08 }}
            >
              <span className="led-light red-on" style={{ width: "5px", height: "5px" }} />
              {s.icon}
              {s.label}
            </motion.a>
          ))}
        </div>
      </div>
    </section>
  );
}
